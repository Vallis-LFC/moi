...
string create_commit(const string &commit_content){
    ...
    filesystem::path obj_dir = filesystem::path(".git")/"objects"/subdir_name;
    filesystem::create_directories(".git/objects/"+subdir_name);
    filesystem::path full_path = obj_dir / commit_object_filename;
    ...
    ofstream output_file(full_path, ios::binary);  //ios::binary tells it not to touch characters as they might open in text mode by default
    if (!output_file){
        cerr<<"failed to write commit obj to db path: " <<full_path.string()<< endl;
        return "";
    }
