...
string write_tree(const filesystem::path &dir){
    ...
    for(const auto &entry : filesystem::directory_iterator(dir)){
        ...
        if(name == ".git" || name == "moi.exe") continue;
        string mode;
        string obj_hash;
        ...
        string final_content = "tree "+to_string(tree_entries.size());
        final_content.push_back('\0');
        final_content.append(tree_entries);
        ...
        filesystem::path object_dir = filesystem::path(".git") / "objects" / sub_dir_name;
        filesystem::create_directories(object_dir);
        filesystem::path tree_object_path = object_dir / object_file_name;
