...
string get_git_author() {
    ...
    // Fallback must include <email> format so git fsck / GitHub won't reject the commit
    return "Developer <dev@example.com>";
}

string get_current_timestamp(){
    time_t now = time(nullptr);
    string seconds = to_string(now);
    string time_zone_offset = "-0400";

    return seconds+" "+time_zone_offset;
}
